...
But here's the catch: those 220 CASRN map to just 215 unique CIDs.

```{r}
length(unique(main_cid_cl$cid))
```
...
```{r}
sum(duplicated(ex_cid_chem_all$CID))
```
...
Master list_waterpipe chemical database (for the EX program and waterpipe fillers only)_2025.xlsx = A 

New List EX-clean version.xlsx = B

solution = B -A = 33 

orange_plus = solution + orange (that were originally in A)

We want:
chem_to_eval = Orange_plus – A  (pretty much we check the one that are not in A)

In the past I did B – (A + orange (is bigger)) = 24 but that is not what she wants.
...
orange_plus |> 
  filter(!InChIKey %in% ex_cid_chem_all$InChIKey)
```

<br>


Just in case, we check the chemicals that are in ex_cid_chem_all. but not in orange_plus.



```{r}
ex_cid_chem_all |> 
  filter(!InChIKey %in% orange_plus$InChIKey)
```
